...
export default function RecipesPage() {
  ...
  const filteredRecipes = recipes.filter((recipe) => {
    const cuisineMatch =
      selectedCuisines.length === 0 || selectedCuisines.includes(recipe.strArea);
  ...
  return (
    <div className="bg-orange-50 min-h-screen font-sans flex">
      {/* Sidebar */}
      <aside className="w-64 bg-orange-100 p-6">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-3 md:grid-cols-2 gap-6">
            {filteredRecipes.map((recipe) => (
              <div
                key={recipe.idMeal}
                className="bg-white rounded-sm shadow-md overflow-hidden hover:shadow-lg transition"
              >
                <img
